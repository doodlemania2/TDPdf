...
try {
    ...
    try {
        ...
        if ($intunewin) {
            ...
            Write-Host   ""
            Write-Host   "    Detection rule (Registry - recommended):"
            Write-Host   "      Hive            : HKEY_LOCAL_MACHINE"
            Write-Host   "      Key path        : Software\TDPdf"
            Write-Host   "      Value name      : Version"
            Write-Host   "      Detection method: Value comparison"
            Write-Host   "      Data type       : Version"
            Write-Host   "      Operator        : Greater than or equal to"
            Write-Host   "      Value           : $assemblyVersion"
            Write-Host   "      32-bit on 64    : No"
            Write-Host   ""
            Write-Host   "    Fallback detection rule (File):"
            Write-Host   "      Path            : C:\Program Files\TDPdf"
